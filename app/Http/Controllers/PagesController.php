...
class PagesController extends Controller
{
    public function index()
    {
        ...
        // $pajak = $this->pajak();
        // $grafik_inv = $this->grafik_inv();
        // $grafik_pjk = $this->grafik_pajak();

        // $saldo = $this->showBalanceSaldo(1);
        // $saldo_jpy = $this->showBalanceSaldo(2);
        // $saldo_usd = $this->showBalanceSaldo(3);
        ...
        return view('pages.dashboard.dashboard', compact('page_title','accessMenu', 'page_description'));
    }
    ...
    public function generatePdf(){
        ...
        //return view('pages.master-barang.print_barcode', compact('title','data', 'barang'));

        $pdf = PDF::loadview('pages.master-barang.print_barcode',$data);
        return $pdf->stream();
    }
    ...
    public function viewVirtualHost()
    {
        $file = "virtual host.pdf";
        $path = public_path('arsip/faq/'. $file);
        // header
       $header = [
         'Content-Type' => 'application/pdf',
         'Content-Disposition' => 'inline; filename="' . $file . '"'
    ...
    public function generateMenu(){
        ...
        foreach ($menu as $key => $val) {
            ...
            //     $length = 19;
            // }
            if($val->id==1 OR $val->id==2 OR $val->id==7 OR $val->id==10 OR $val->id==11){
                $length = 1;
            }
            else{
                $length = 4;
        ...
        $menu_act = DB::table('sys_menu_actions')->get();
        foreach ($menu_act as $key => $value) {
            $index_act =array(
                'role_id'           =>1,
                'menu_action_id'    =>$value->id,
            );
            DB::table('sys_role_menu_actions')->insert($index_act);
        }
