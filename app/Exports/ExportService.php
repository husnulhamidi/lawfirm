...
class ExportService implements FromView
{
    private $request;
    ...
    public function view(): View
    {
        ...
        //         ->get();
        $service = Transaksi::with(['pelanggan', 'piutang',
                        'detail_list'=> function ($builder) {
                            $builder->with(['barang']);
                        }
                    ]   
                )
        ...
        $subtotal = Transaksi::whereBetween('tanggal', [$startDate, $endDate])->where('is_transaksi',0)->where('is_complete','1')->sum('subtotal') ?? 0;
        $diskon = Transaksi::whereBetween('tanggal', [$startDate, $endDate])->where('is_transaksi',0)->where('is_complete','1')->sum('diskon_total') ?? 0;
        $grand_total = Transaksi::whereBetween('tanggal', [$startDate, $endDate])->where('is_transaksi',0)->where('is_complete','1')->sum('grand_total') ?? 0;
        $totalTF = Transaksi::whereBetween('tanggal', [$startDate, $endDate])->where('is_transaksi',0)->where('is_complete','1')->sum('transfer') ?? 0;
        $totalCash = Transaksi::whereBetween('tanggal', [$startDate, $endDate])->where('is_transaksi',0)->where('is_complete','1')->sum('cash') ?? 0;
        ...
        //echo response()->json($service);die;
        return view('pages.exports.service', [
            'service' => $service,
